        ...
        with open(path) as file:
            ...
            for product in data:
                ...
                dataParsed.append({
                    ...
                    'nome_do_produto': tag.css('nome_do_produto::text').get(),
                    'nome_da_empresa': tag.css('nome_da_empresa::text').get(),
                    'data_de_fabricacao': tag.css(
                        'data_de_fabricacao::text').get(),
                    'data_de_validade': tag.css(
                        'data_de_validade::text').get(),
                    'numero_de_serie': tag.css('numero_de_serie::text').get(),
        ...
        with open(path) as file:
            ...
            for product in data:
                ...
                for index in range(len(product)):
                    dictionary[header[index]] = product[index]
                    dataParsed.append(dictionary)
        return dataParsed
