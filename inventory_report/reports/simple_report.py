...
class SimpleReport(Report):
    ...
    def generate(cls, reports):
        def oldest_fabrication_date_sort(report):
            fabrication_date = datetime.strptime(
        ...
        oldest_fabrication_date = sorted(
            reports, key=oldest_fabrication_date_sort
        )[0]["data_de_fabricacao"]
        closest_validity_date = sorted(
            reports, key=closest_validity_date_sort
        )[0]["data_de_validade"]
...
Empresa com maior quantidade de produtos estocados: {company_products}
"""
